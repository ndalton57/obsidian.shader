...
// NOTE: volumetric clouds are deliberately NOT included here. The sky map
// feeds environment reflections and the d4a ambient SH that lights all
// terrain — cloud cover must not change ambient light (and with it shadow
// brightness). Clouds are composited per-pixel in d4; storm darkening comes
// from the cloud shadow map.
...
#if defined PROGRAM_DEFERRED0
// Aurora only — clouds intentionally stay out of the sky map (see the note
// above the draw_sky helpers)
vec4 get_clouds_and_aurora(vec3 ray_dir, out float clouds_apparent_distance) {
...
vec3 draw_sky(vec3 ray_dir) {
    ...
    float clouds_apparent_distance;
    vec4 clouds_and_aurora
        = get_clouds_and_aurora(ray_dir, clouds_apparent_distance);
    return draw_sky(
        ray_dir,
        atmosphere,
        clouds_and_aurora,
...
vec3 draw_sky(vec3 ray_dir) {
    ...
    sky += 0.1 * ambient_color * mie_phase;
